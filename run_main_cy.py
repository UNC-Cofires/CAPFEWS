...
### make new initialization for CAP model
if model_mode == 'central_arizona_project':
  if redo_init == 1:
    ### setup/initialize model
    print('#######################################################')
    print('Begin initialization of CAPFEWS model...')
    sys.stdout.flush()
...
if model_mode == 'simulation' or model_mode == 'validation':
  if redo_init == 1:
    ...
    print('If you meant to run the CAP model, remember to change the runtime_params.ini file...')
    sys.stdout.flush()
